...
def PDW_Params(parser):
    """
    This function has been written in an untidy way, it rewrites all parameters used by the specific OOD algorithms.
    Should be rewritten, if a better way can be found.
    """
...
def PDW_Training(
    ...
    features_all_classes, class_identifiers = tools.features_to_dim(
        features_all_classes, classes_to_process=None
    )
    ...
    pos_classes_to_process = pos_classes_to_process[
        gpu * div + min(gpu, mod) : (gpu + 1) * div + min(gpu + 1, mod)
    ]
    print(f"Processing classes {pos_classes_to_process}")

    algo_iterator = OOD_Method(
        pos_classes_to_process, features_all_classes, args, gpu, models
    )
...
def PDW_Inference(
    ...
    # Convert dict from based on classes to based on dimension, making each dimension a class
    features_all_classes, class_identifiers = tools.features_to_dim(
        features_all_classes, classes_to_process=pos_classes_to_process
    )
    ...
    models = opensetAlgos.heuristic.set_shape_scale_defaults(
        models, set_shape_to=args.set_shape_to, set_scale_to=args.set_scale_to
    )
