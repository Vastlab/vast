...
class ExternalInputIterator():
    def __init__(self, batch_size, device_id, num_gpus, filenames, images_path,
                 debug=False, random_seed=None, unknowns_label=-1, return_file_names=False):
    ...
    def __next__(self):
        batch_inputs = []
        batch_labels = []
        if self.return_file_names: batch_file_names = []
        if self.i >= self.n:
            raise StopIteration
        while len(batch_inputs)<self.batch_size and self.i<self.n:
            ...
            if self.return_file_names:
                name = jpeg_path.split('/')[-1]
                name = int(name.split('.')[0].split('_')[-1])
                batch_file_names.append(np.array([name]).astype(np.int32))
#                print ("batch_file_names",batch_file_names)
        ...
        if self.i==self.n:
            ...
            while len(batch_inputs) < self.batch_size:
                ...
                batch_inputs.append(np.frombuffer(f.read(), dtype=np.uint8))
                batch_labels.append(np.array([label], dtype=np.uint8))
                if self.return_file_names: batch_file_names.append(np.array([-1]).astype(np.int32))
                counter+=1
        if self.return_file_names:
            return (batch_inputs, batch_labels, batch_file_names)
...
class ExternalSourcePipeline(Pipeline):
    def __init__(self, batch_size, num_threads, device_id, external_data, device_type="gpu",
                 training=False, image_size=224, return_file_names=False):
        ...
        self.return_file_names = return_file_names
        if self.return_file_names:
            self.input_filenames = nvidia_ops.ExternalSource()
        ...
        # self.file_names_cast = nvidia_ops.Cast(device="cpu", dtype=nvidia_types.STRING)
    ...
    def define_graph(self):
        ...
        if self.return_file_names:
            self.file_names = self.input_filenames()
        ...
        if self.return_file_names:
            return (output, self.labels, self.file_names)
        # return (output, self.labels, self.file_names_cast(self.file_names))
        return (output, self.labels)

    def iter_setup(self):
        try:
            if self.return_file_names:
                (images, labels, file_names) = self.iterator.next()
                self.feed_input(self.jpegs, images)
                self.feed_input(self.labels, labels)
                self.feed_input(self.file_names, file_names)
            else:
